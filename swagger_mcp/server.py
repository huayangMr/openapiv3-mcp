...
def get_api_details(path: str, method: str) -> Dict[str, Any]:
    ...
    if api.request_body:
        request_body = {
            ...
            "required": api.request_body.required,
            "content_type": api.request_body.content_type,
            "schema": api.request_body.schema,
            "content": api.request_body.content
        }
...
def run_server():
    """运行 MCP 服务，并可在启动时加载 OpenAPI/Swagger 文档"""
    arg_parser = argparse.ArgumentParser(description="Run Swagger MCP server")
